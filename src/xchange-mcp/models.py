...
class InitExchangeInput(BaseModel):
    ...
    api_password: Optional[str] = Field(
        default=None, description="API password/passphrase (OKX, Bitget, etc.)"
    )
    is_testnet: bool = Field(default=False, description="Use testnet/sandbox")
    market_type: str = Field(
        default="swap", description="Market type: spot, swap, margin, stock"
    )
    exchange_id: Optional[str] = Field(
        default=None, description="Optional exchange identifier override"
    )
    user_id: Optional[str] = Field(default=None, description="Optional user identifier")
    sub_account_id: Optional[str] = Field(
        default=None, description="Sub-account identifier"
    )
    symbol: Optional[str] = Field(
        default=None, description="Default symbol (e.g. 'BTC/USDT')"
    )
    leverage: Optional[int] = Field(default=None, description="Default leverage")
    margin_mode: Optional[str] = Field(
        default=None, description="Margin mode: cross or isolated"
    )
    position_mode: Optional[str] = Field(
        default=None, description="Position mode: one_way or hedge"
    )
...
class CreateOrderInput(BaseModel):
    ...
    params: Optional[dict[str, Any]] = Field(
        default=None, description="Extra exchange-specific parameters"
    )
...
class CancelAllOrdersInput(BaseModel):
    session_id: str
    symbol: Optional[str] = Field(
        default=None, description="Symbol to cancel orders for; all symbols if omitted"
    )
...
class ClosePositionInput(BaseModel):
    ...
    amount: Optional[float] = Field(
        default=None, description="Amount to close; full position if omitted"
    )
...
class FetchOhlcvInput(BaseModel):
    ...
    symbol: str = Field(..., description="Trading pair")
    timeframe: str = Field(
        default="1h", description="Timeframe (e.g. '1m', '5m', '1h', '1d')"
    )
    since: Optional[int] = Field(
        default=None, description="Start timestamp in milliseconds"
    )
...
class FetchMyTradesInput(BaseModel):
    ...
    limit: Optional[int] = Field(
        default=50, description="Max number of trades to return (default 50)"
    )
...
class GetFilledOrdersInput(BaseModel):
    session_id: str
    symbol: Optional[str] = Field(default=None, description="Symbol filter")
    since: Optional[int] = Field(
        default=None, description="Start timestamp in milliseconds"
...
class GetBorrowRateInput(BaseModel):
    session_id: str
    asset: str = Field(..., description="Assetsymbol to query (e.g. 'BTC', 'USDT')")


class GetBorrowedAmountInput(BaseModel):
    session_id: str
    asset: str = Field(..., description="Asset symbol to query (e.g. 'BTC', 'USDT')")


class GetBorrowedRecordsInput(BaseModel):
    session_id: str
    asset: str = Field(..., description="Asset symbol to query (e.g. 'BTC', 'USDT')")


class FetchMarginConfigInput(BaseModel):
    session_id: str
    symbol: Optional[str] = Field(default=None, description="Trading pair")


class IsTestnetInput(BaseModel):
    session_id: str
